...
class RolesCog(commands.Cog):
    ...
    @app_commands.describe(
        channel="Channel (or category) whose permissions to copy",
    ...
    async def nexcopyperms(
        ...
        if not channel.permissions_for(me).manage_roles:
            await interaction.followup.send(
                ...
                f"{channel.mention} to edit its permission overwrites.",
                ephemeral=True,
            )
            return

        overwrite = channel.overwrites_for(source)
        had_existing = target in channel.overwrites

        try:
            await channel.set_permissions(
                target,
                overwrite=overwrite,
                reason=f"/nexcopyperms by {interaction.user} (copied from @{source.name})",
            )
        except discord.Forbidden:
            await interaction.followup.send(
                f"{emoji('nex_error')} Discord refused the edit — make sure my role is high "
                f"enough and has **Manage Roles** in {channel.mention}.",
                ephemeral=True,
            )
            return
        ...
        lines = [
            f"{emoji('nex_success')} Copied {source.mention}'s permissions to "
            f"{target.mention} in {channel.mention} — {n_allow} allowed, {n_deny} denied."
        ]
        if had_existing:
            lines.append(f"-# Replaced {target.name}'s existing settings in this channel.")
